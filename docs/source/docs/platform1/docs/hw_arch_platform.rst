...
** If the media file is transfered from host x86 to the VMK180 evaluation board (endpoint) through the PCIe Queue DMA (QDMA).
...
** For the HDMI dispay on host monitor the design uses the CPM PCI Express (PCIe) Endpoint Hard block in an Gen3 x8 configuration along with QDMA for data transfers between the endpoint and the host system memory . The QDMA provides protocol conversion between PCIe transaction layer packets (TLPs) and AXI transactions. The DMA cores are used for data transfer between the memory controller (DDR) in PL to the host memory . AXI Bridge core in the CPM is used for AXI-to-host communication. AXI4-Lite slaves has the user space registers which are used for hand shaking between host and endpoint.
...
Hardware Architecture of the host to end point 
------------------------------------------------
